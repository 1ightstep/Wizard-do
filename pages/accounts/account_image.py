    ...
    def create_icons(self, account_page):
        ...
        for row in range(5):
            for col in range(5):
                ...
                image = ttk.Label(self, text=img, image=icon)
                setattr(image, f"{self.pictures[img]}", img)
                image.image = icon
                image.pack_propagate(False)
                image.grid(row=row, column=col, padx=2, pady=2)
                image.bind("<Button-1>", lambda _, icon_var=icon: account_page.update_icon(icon_var, image.cget("text")))
