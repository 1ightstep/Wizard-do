...
class AccountMenu(ttk.LabelFrame):
    def __init__(self, master, account_page, dashboard_page, tasks_page):
        ...
        self.sign_up = ttk.Button(self,
                                  text="Sign Up",
                                  padding=5,
                                  command=lambda: account_create.AccountCreate(account_page, dashboard_page, tasks_page))
        self.sign_in = ttk.Button(self,
                                  text="Sign In",
                                  padding=5,
                                  command=lambda: self.account_in(account_page, dashboard_page, tasks_page))
        ...
        self.sign_out = ttk.Button(self,
                                   ...
                                   padding=5,
                                   width=45,
                                   command=lambda: self.sign_out_cmd(account_page, dashboard_page, tasks_page))
        self.del_account = ttk.Button(self,
                                      text="Delete Account",
                                      padding=5,
                                      width=45,
                                      command=lambda: self.delete_account(account_page, dashboard_page, tasks_page))
    ...
    def account_in(self, account_page, dashboard_page, tasks_page):
        try:
            if self.database.return_all("accounts"):
                account_in.AccountIn(account_page, dashboard_page, tasks_page)
            else:
    ...
    def sign_out_cmd(self, account_page, dashboard_page, tasks_page):
        if self.database.return_value("settings", "signed_in"):
            if messagebox.askyesno("Confirm Sign Out", "Are you sure you want to sign out?"):
                ...
                account_page.update_ui("Guest", tasks_page)
